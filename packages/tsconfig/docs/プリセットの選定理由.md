# プリセットの選定理由

`src/` の各プリセットの「なぜこの値にしたか」を残す。設定が何をするかの一行説明は各 JSON のコメントに書き、ここには理由と参照リンクだけを書く。

## 前提

- このプリセットは TypeScript 6+ 前提。TS 6.0 でデフォルト true になった `strict` と `noUncheckedSideEffectImports` は指定しない
- `strict` を明示していた理由は `exactOptionalPropertyTypes` の警告バグ [microsoft/TypeScript#63232](https://github.com/microsoft/TypeScript/issues/63232) だったが、6.0.3 での修正を再現テストで確認して外した。[TS 6.0 のデフォルト変更一覧](https://devblogs.microsoft.com/typescript/announcing-typescript-6-0/)
- `noUncheckedSideEffectImports` により副作用 import の解決失敗はエラーになる。CSS などのアセットを import する project は `declare module "*.css";` のアンビエント宣言を追加する。[noUncheckedSideEffectImports](https://www.typescriptlang.org/tsconfig/#noUncheckedSideEffectImports)
- tsc は typecheck 専用 (`noEmit: true`) にして、emit はバンドラー (tsdown / tsup / esbuild など) に任せる。tsc で emit する構成は `tsconfig.tsc.json` と `tsconfig.lib.json` だけが `noEmit: false` で明示する。この 2 つは base の `allowImportingTsExtensions` も `false` に戻す。`noEmit` / `emitDeclarationOnly` / `rewriteRelativeImportExtensions` のどれも無いと tsc が TS5096 で拒否するため

## tsconfig.json (base)

- `skipLibCheck: true` — false だと node_modules の .d.ts まで全部チェックしてパフォーマンスが悪い。[解説動画](https://www.youtube.com/watch?v=zu-EgnbmcLY)
- `target: ES2025` — ES2025 で Iterator helpers / Set methods / `Promise.try` などの型が使える。Node 22 LTS / Node 24 / 主要モダンブラウザは 2026 年時点でネイティブサポート済み。Node バージョンとの対応は [Node Target Mapping](https://github.com/microsoft/TypeScript/wiki/Node-Target-Mapping)
- `module: "preserve"` — imports / exports を変換せず保持し、`moduleResolution: "bundler"` を暗黙指定する (TS 5.4+)。tsc 以外が transpile する現状の標準構成に合う。tsc で transpile する場合は `tsconfig.tsc.json` が `NodeNext` に上書きする。[module: preserve](https://www.typescriptlang.org/tsconfig/module.html#preserve)
- `allowImportingTsExtensions: true` — Vite は 9 で設定ファイルを Node に直接読ませる `configLoader: 'native'` を[既定にする計画](https://github.com/vitejs/vite/issues/21546)で、Node の ESM は相対 import に拡張子を要求する。Vite 8.2.0 から `vite.config.ts` とそこから相対 import するファイルの[拡張子なし相対 import が警告になる](https://github.com/vitejs/vite/pull/22850)。[create-vite のテンプレート](https://github.com/vitejs/vite/blob/main/packages/create-vite/template-react-ts/tsconfig.node.json)も `vite.config.ts` 用の tsconfig に同じ値を入れている。拡張子なしの import はこれまでどおり通り、`.ts` 付きを強制はしない。emit しない前提なので `rewriteRelativeImportExtensions` は採らない。base を extends して `noEmit: false` を上書きする構成は TS5096 になるので、`tsconfig.tsc.json` / `tsconfig.lib.json` に移るか `allowImportingTsExtensions: false` も上書きする
- `moduleDetection: "force"` — 全ファイルがモジュール扱いになり、ファイルがスコープの単位になるので別ファイルで同名の変数を定義できる。[解説](https://www.totaltypescript.com/cannot-redeclare-block-scoped-variable)
- `isolatedModules: true` — tsc 以外のトランスパイラはファイル単位でしか変換できないため、その前提で書けているかを検査させる
- `verbatimModuleSyntax: true` — import type / export type の明示で可読性が上がり、トランスパイル後に不要な import が残らない
- `erasableSyntaxOnly: true` — enum / runtime namespace / parameter properties をエラー化し、Node 23.6+ の type-stripping や tsgo でも実行できる構文に制限する (TS 5.8+)。`verbatimModuleSyntax: true` と方針が揃う。[TS 5.8 announcement](https://devblogs.microsoft.com/typescript/announcing-typescript-5-8/)
- `noUncheckedIndexedAccess: true` / `exactOptionalPropertyTypes: true` — strict に含まれない追加の strict 系。インデックスアクセスと optional property の `undefined` を厳密に扱う
- `lib: ["ES2025", "ESNext.Disposable"]` — `lib: ["ESNext"]` にすると Temporal などランタイム未実装の API まで型が通ってしまうため、`ES2025` を基準にして必要なものだけ個別に足す方針を取る。`ESNext.Disposable` は `using` / `await using` の型で、Node は 24.0.0 でフル実装済み ([MDN の互換データ](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol/dispose))。ブラウザは Chrome / Edge 134+、Firefox 141+ で Safari が全バージョン未対応 ([caniuse](https://caniuse.com/wf-explicit-resource-management)) のため、client code で使うときはランタイム対応を確認する。DOM 系プリセットは `lib` を上書きするためこの指定を継承しない。`tsconfig.nextjs.json` は `ESNext` に含まれ、`tsconfig.tanstack-start.json` は明示的に足す

## tsconfig.tsc.json

tsc で transpile する構成。base の `module: "preserve"` を `NodeNext` に上書きし、`noEmit: false` で emit を有効化する。

## tsconfig.lib.json

- `isolatedDeclarations: true` — tsdown / tsup / rolldown-plugin-dts などが .d.ts を並列生成できるようになる。代わりに全 export へ明示的な型注釈が必要。[isolatedDeclarations](https://www.typescriptlang.org/tsconfig/isolatedDeclarations.html)
- `allowJs: false` — tsc は `isolatedDeclarations` と `allowJs` の同時指定を TS5053 で拒否するため、base の `allowJs: true` を打ち消す。workspace の package は tsdown で build していて lib preset を tsc に直接渡さないので、CI ではこの組み合わせを検知できない

## tsconfig.nextjs.json

- `jsx: "react-jsx"` — Next.js が React automatic runtime を使うため、[writeConfigurationDefaults.ts](https://github.com/vercel/next.js/blob/canary/packages/next/src/lib/typescript/writeConfigurationDefaults.ts) で「ユーザーが変更できない必須値」として定義されている。`module` などの他の必須値と違い許容値リストを持たず `react-jsx` 一択で、`create-next-app` の生成する tsconfig も同じ値。以前は `preserve` を指定していたが、これは Next.js 11 以前の前提だった
- `lib: ["dom", "ESNext"]` — `dom.iterable` は TypeScript 6.0 で `dom` にマージ予定のため指定しない。[比較 gist](https://gist.github.com/privatenumber/3d2e80da28f84ee30b77d53e1693378f)

## tsconfig.tanstack-start.json

- `module` / `moduleResolution` — [公式の build-from-scratch 手順](https://tanstack.com/start/latest/docs/framework/react/build-from-scratch)の案内どおりの値
- `target` を指定しない — 公式が案内する `ES2022` は "at least" の最低要件で、base の `ES2025` が満たす。Vite は tsconfig の `target` を無視して `build.target` / `oxc.target` を使うため、値を上げても出力は変わらない。[Vite の TypeScript](https://vite.dev/guide/features.html#typescript)
- `lib: ["DOM", "DOM.Iterable", "ES2025", "ESNext.Disposable"]` — base の `ES2025` に DOM 系を足す。`ES2022` のままでは `Promise.withResolvers` など ES2023 以降の型が引けない。`ESNext.Disposable` は `lib` の上書きで base から消えるので足し直す。TanStack Start の app では Playwright で OG 画像を作るような Node で動くスクリプトも同じ tsconfig で typecheck するため、`await using browser = await chromium.launch()` の型が要る
- `verbatimModuleSyntax: false` — server module が client bundle に残る可能性があるため、公式が無効を案内している
- `types: ["vite/client"]` — asset import / `import.meta.env` / `import.meta.hot` の型を足す。[Vite client types](https://vite.dev/guide/features.html#client-types)
